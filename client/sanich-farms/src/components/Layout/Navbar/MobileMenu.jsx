            ...
            {isAuthenticated ? (
              <>
                <div className="px-4 py-3 mt-4 border-t border-gray-100">
                  <span className="text-green-600 font-bold text-base">Hi! {user?.name?.split(' ')[0] || 'User'}</span>
                </div>
                <button
                  onClick={onLogout}
                  className="block w-full text-center bg-red-600 text-white px-4 sm:px-6 py-3 sm:py-4 rounded-lg sm:rounded-full text-base sm:text-lg font-semibold hover:bg-red-700 active:bg-red-800 transition-all duration-300 mt-4 shadow-md hover:shadow-lg min-h-[48px] touch-manipulation"
                >
